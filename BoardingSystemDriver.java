...
public class BoardingSystemDriver {
  ...
  protected void enqueuePassenger(Passenger p) {
    ...
    boolean enqueueAttempt = boardingQueue.enqueue(p);
    if (enqueueAttempt == false) {
  ...
  protected void dequeuePassenger() {
    ...
    // [HINT] use BoardingQueue.dequeue() to dequeue the passenger to board the
    // airplane next
    // [HINT] use Passenger.isCheckedIn() to check whether the dequeued passenger
    // has already
    // checked in
  ...
  // TODO comment out the implementation details of startBoarding() method after
  // you implement the
  ...
  private Passenger getPassengerToAdd(String commandLine) {
    ...
    try {
      ...
      // and return it
      Passenger newPassenger = new Passenger(name, group, isCheckedIn);
      return newPassenger; // default return statement. Feel free to change it
      ...
      try {
        switch (command.charAt(0)) {
          ...
          case '2': // [2] Peek next passenger to board"
            ...
            Passenger p2 = boardingQueue.peekBest();
